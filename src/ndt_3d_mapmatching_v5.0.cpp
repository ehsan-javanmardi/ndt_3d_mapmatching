...
2018.01.07
    ndt_3D_mapmatching_v5.0.3
    added log file for debug

2018.01.04
    ndt_3D_mapmatching_v5.0.2
    Load map in main
    solved some minor error in show_in_rviz

2018.01.02
    ndt_3D_mapmatching_v5.0.1
    tf_lidar angles are changed immidiately after getting from param
    Added show in rviz instead of all other show_XXX
...
double vel_x, vel_y, vel_z, vel_roll, vel_pitch, vel_yaw = 0.0; // angles are in radian
...
static void scan_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
    ...
    if (input->header.seq - previous_seq != 1)
    {
        fprintf(pFileLog, "previous sequence is : %i and current is : %i so we had error!!", input->header.seq, previous_seq);
    }
    ...
    pose pose_lidar(vel_x, vel_y, vel_z, vel_roll, vel_pitch, vel_yaw);
...
*/
    ...
    if (true)
    {
        ...
        FILE* pFile;
        pFile = fopen(pose_file_path.c_str(), "a");
...
int main(int argc, char **argv)
{
    ...
    vel_roll = (vel_roll /180.0) * M_PI;
    vel_pitch = (vel_pitch /180.0) * M_PI;
    vel_yaw = (vel_yaw /180.0) * M_PI;
    ...
    private_nh.getParam("show_in_rviz", show_in_rviz);
    ...
    ndt_map_pub = nh.advertise<visualization_msgs::MarkerArray>("ndt_map", 1000);

    ros::Subscriber scan_sub = nh.subscribe(in_vel_topic, 15000, scan_callback);
    ...
    if (show_in_rviz)
    {
        // publish down sampled map on rviz
        ...
        //std::cout << "Down-sampled map size is : " << filtered_map.size() << std::endl;
    ...
    pose_file_path = save_path + "vehicle_pose.csv";

    FILE *pFile;

    pFile = fopen (pose_file_path.c_str(), "w");
    ...
    std::string name = save_path + "log.csv";
