...
Basic map matching with NDT class
 ...
 2017.12.21
    ndt_3d_mapmatching_groundtruth_ds_ndt_same_phase_v1.0
    based on ndt_3d_mapmatching_groundtruth_v1.0
    The input scan is downsampled with the phase comes from ground truth:
        first the scan is moved to ground tyruth and then downsampled and then again transfomed to base_link and then matched!!


 2017.12.21
    ndt_3d_mapmatching_groundtruth_v1.0
    based on ndt_3D_mapmatching_v5.0
...
bool groundtruth_as_prediction = true;
...
static void scan_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
    ...
    publish_pointCloud(calibrated_scan_xyzir, calibrated_scan_pub, "/map");
...
*/
    publish_pointCloud(scan, scan_pub, "/map");
...
*/
    ...
    // transformed by groundtruth guess

    pcl::PointCloud<PointXYZI> groundtruth_scan;

    pcl::transformPointCloud(scan, groundtruth_scan, tf_predict); //which is groundtruth

    pcl::PointCloud<PointXYZI>::Ptr input_cloud_ptr(new pcl::PointCloud<PointXYZI>(groundtruth_scan));
    ...
    // bring it back to base_link

    //pose predict_reverse_pose;

    //predict_reverse_pose = predict_pose.inverse();

    /*predict_reverse_pose.x = -1.0 * predict_pose.x;
    predict_reverse_pose.y = -1.0 * predict_pose.y;
    predict_reverse_pose.z = -1.0 * predict_pose.z;
    predict_reverse_pose.roll = -1.0 * predict_pose.roll;
    predict_reverse_pose.pitch = -1.0 * predict_pose.pitch;
    predict_reverse_pose.yaw = -1.0 * predict_pose.yaw;*/

    Eigen::Matrix4f tf_predict_reverse;

    tf_predict_reverse = tf_predict.inverse();

    //pose_to_tf(predict_reverse_pose, tf_predict_reverse);

    pcl::transformPointCloud(filtered_scan, filtered_scan, tf_predict_reverse); //which is groundtruth

    // show the filtered scan just for debug
...
{
    std::cout << "ndt_3d_mapmatching_groundtruth_ds_ndt_same_phase_v1_0\n" ;
    ros::init(argc, argv, "ndt_3d_mapmatching_groundtruth_ds_ndt_same_phase_v1_0");
    ...
    save_path = save_path + str_time + "ndt_mapmatching_groundtruth_samedsphse" + "/";
    ...
    filtered_scan_pub = nh.advertise<sensor_msgs::PointCloud2>("/ds_same_phase/filtered_scan", 1000);
